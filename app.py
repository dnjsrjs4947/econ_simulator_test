...
from visual import plot_time_series, plot_metric

st.set_page_config(page_title="경제 시뮬레이터", layout="wide")
...
# 3) 사이드바: 정책 파라미터 입력
st.sidebar.header("정책 파라미터")
policy = PolicyInput(
    interest_rate   = st.sidebar.number_input("기준금리 (%)", value=1.5, step=0.1),
    corporate_tax   = st.sidebar.number_input("법인세율 (%)", value=25.0, step=1.0),
    electricity_cost= st.sidebar.number_input("공업용 전기요금 (원/kWh)", value=100.0, step=1.0),
    exchange_rate   = st.sidebar.number_input("원-달러 환율", value=1200.0, step=1.0),
)
